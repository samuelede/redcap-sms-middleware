...
5. **PHP saves answer & sends next question** until **q5b** is completed for the day
...
| `secure/secrets.php`               | **git-ignored** secrets: REDCap token/URL, provider keys, VMNs, static JWT |
| `api/send_outbound.php`            | Outbound engine: evaluates q1a window, reminders, auto-heal; sends SMS |
| `api/send_inbound.php`             | Inbound parser: validates/saves reply; re-invokes outbound to continue sequence |
...
> **.gitignore should include:** `/secure/`, `/logs/`, and any other environment-specific outputs.
